...
export const PASSENGERS = [
  ...
  { id:'waxer',    name:'清潔工', fare:0, patience:999, size:1, w:0, band:'retail',
    note:'地板打蠟的工班。送到那一層之後，那層樓才開始封閉——不送，就不會封。' },
];
...
export const EVENTS = [
  ...
  //   hours:[5,10] —— 商場開門前打蠟。零售帶自己的尖峰窗是 [11,20]/[11,21]，所以封鎖
  //                   永遠不會落在**這一帶**的尖峰裡；但 8–10 點是全樓的上班尖峰
  //                   （rushMult 3.0），所以「封鎖 × 尖峰」這件事會發生、可以量。
  //                   附帶：隨機事件池在 00:00–07:00 是空的（沒有一列 hours 蓋到），
  //                   這一列同時補上 05:00–07:00 那個洞。
  //   w:12         —— 現有權重 5–30；這是反覆出現的干擾，不是頭條，所以取在
  //                   消防演習(10) 與到貨潮(14) 之間。實際頻率的瓶頸是 hours 不是 w。
  //   block:[18,30]—— 下限 18 秒 ≈ 早期樓一趟來回；上限 30 秒 ≈ 上班族耐性 46 秒的
  //                   65%，也是一天 180 秒的六分之一。所以「本來就在等的人」有真的
  //                   風險、剛到的人通常撐得過，而整天的損失上限是那一層的 1/6。
  //   n:[1,2]      —— owner 指定 1–2 個清潔工。他們是**真的乘客**，要送。
  //   inbound + blockOn:'deliver'
  //                —— 順序是「人到了才開始打蠟」。這也讓封鎖變成玩家自己造成的：
  //                   不送他們上去就不會封（但他們耐性 999，不走，只是延後）。
  { id:'waxing', name:'地板打蠟', w:12, n:[1,2], at:'retail', to:'lobby',
    inbound:true, type:'waxer', hours:[5,10], block:[18,30], blockOn:'deliver',
    text:'🧴 {f} 樓要打蠟：{n} 個清潔工在大廳等，送上去之後那層會封閉 {s} 秒',
