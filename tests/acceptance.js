...
const SPEC = {
  ...
  //
  // ---- #162：`rateExp` 0.65 → 0.7（`bandPops` 與 `ratePerWeight` 不動）----
  // **owner 裁決（#162）：「`RATE_EXP` 改成 0.7」**，理由是 owner 回報「100 層、
  // 升滿級、6 個電梯，都可以 100%」——終期不該還是 100%。k=1.5 明文留原樣，
  // 所以現在是 #146 網格上的 `0.70/1.5`（該網格唯一的 5/6）。
  // ⚠ **這一格與產品又是同一個 commit 改的**（peer，#162），所以下面那條
  // `C.RATE_EXP !== SPEC.rateExp` 在這一筆上驗不到東西——擋它的還是上面那句裁決。
  // **紅過的證據（#162）**：產品改 0.7、這裡還是 0.65 的那一趟，第 21 組全綠，
  // 只有這一條紅，逐字是
  // 「人流的旋鈕被動過而抄本沒跟上（1 處）：RATE_EXP 期望 0.65，實際 0.7」。
  //
  // ---- #163：`rateExp` 0.7 → 1.0（`bandPops` 與 `ratePerWeight` 仍然不動）----
  // **owner 裁決（#163）：「`RATE_EXP` 改成 1」**，同一個理由的第四次（0.5 → 0.65 →
  // 0.7 → 1.0）：owner 回報終期還是太簡單。1.0 是**線性**，不再是次線性——
  // #146 那張 5×5 網格最大只掃到 0.70，**1.0 已經在網格之外**。
  // ⚠ **這一格與產品又是同一個 commit 改的**（peer，#163），所以下面那條
  // `C.RATE_EXP !== SPEC.rateExp` 在這一筆上驗不到東西——擋它的還是上面那句裁決。
  // **紅過的證據（#163）**：產品改 1.0、這裡還是 0.7 的那一趟（`js/content.js` 已改、
  // 本檔未改），全域是 **114 通過 / 5 失敗 / 0 尚未實作**（改動前的 cd7d2b7 是
  // 119 / 0 / 0），而這一條的紅訊息逐字是
  // 「人流的旋鈕被動過而抄本沒跟上（1 處）：RATE_EXP 期望 0.7，實際 1」。
  // 另外四條紅**不是抄本沒跟上，是這次改動真的打壞的東西**，寫在這裡是因為
  // 下一個人看到「5 失敗」會想知道剩下四條是什麼：
  //   · 第 21 組七列有六列跌破下限（實測與建議門檻在 #163，**門檻是 owner 的，沒有動**）
  //   · 第 24 組三條（#147 包場）——1.0 之下佇列長期貼著 `WAIT_CAP`，
  //     空車幾乎不存在，包場一趟都封不到，那一組的母體變成空的。
  //     **那不是抄本問題，也不是門檻問題**，報在 #163 上。
  //
  // ---- #166：`rateExp` 1.0 → 0.8（`bandPops` 與 `ratePerWeight` 仍然不動）----
  // **owner 裁決（#166，逐字）：「`RATE_EXP` 改成 0.8。」** 第五次調這個旋鈕
  // （0.5 → 0.65 → 0.7 → 1.0 → 0.8），而且是**第一次往回退**：理由寫在 #166 上——
  // 1.0 不是「變難」是「壓垮」（到達率 5.3 人/秒 vs 六井吞吐 1.5 人/秒、佇列不收斂、
  // 評價塌到地板、收入反而下降）。
  // ⚠ **這一格與產品是同一條分支改的**（peer，#166），所以下面那條
  // `C.RATE_EXP !== SPEC.rateExp` 在這一筆上驗不到東西——擋它的還是上面那句裁決。
  // **紅過的證據（#166）**：本檔已改 0.8、`js/content.js` 還是 1.0 的那一趟，
  // 全域是 **115 通過 / 1 失敗 / 3 尚未實作**（改動前的 b2baf56 是 116 / 0 / 3），
  // 而這一條的紅訊息逐字是
  // 「人流的旋鈕被動過而抄本沒跟上（1 處）：RATE_EXP 期望 0.8，實際 1」。
  ratePerWeight: 0.032,    // §5.x 人流公式的常數
  rateExp: 0.8,            // §5.x 人流指數（#166；#163 是 1.0、#162 是 0.7、#151 是 0.65、之前 0.5）
...
// **這是煙霧測試的門檻，不是配平目標**：抓的是崩塌，不是漂移。
// ---- #163 之後重訂（owner 裁決「甲」）----
// `RATE_EXP` 1.0 之下七列垮六列。owner 看過「這不是變難、是壞掉」的數字之後
// 仍然選擇 land 1.0，並接受重訂門檻。
//
// ⚠ **這七個數字量到的東西，有一部分是 `WAIT_CAP` 不是電梯。** 三個 100 樓的劇本
// 有 60–70% 的 tick 是閘關著的（`sim.js:1304` 佇列滿就整個不生人），而沒出現過的
// 人不進任何分母。把 `WAIT_CAP` 從 160 拉到無上限，同一個劇本的送達率是
// **74.28% → 35.36%，而送達人數幾乎不動（1104 → 1135）**——六井在 100 樓的吞吐
// 上限約 1.5 人/秒，人流是 5.3 人/秒。**上限不是在扭曲一個好數字，是在藏一個更糟的。**
// 所以這七個下限**只在 `WAIT_CAP = 160` 之下有意義**；那個常數一動，這裡要重量。
//
// 實測值：12 顆種子 × 15 遊戲日（#163 的 peer）。下限照既有的「實測 −8～10」慣例。
// 括號裡是 0.7 的實測值，供對照——**舊註解裡那組 48.0/69.8/… 是 0.65 的，#162 沒更新，
// 這一輪一併修掉。**
const THR_CONFIGS = [
  // 名稱                        樓層 up.shaft up.cap  額外自動化   送達率下限
  ['tight 2井cap6 45層',          45,      1,     1,  [],            14],   // 實測 22.80（0.7 時 41.65）
  ['healthy 3井cap8 45層',        45,      2,     2,  [],            26],   // 實測 34.50（0.7 時 65.01）
  ['100層 欠配 3井cap8',         100,      2,     2,  [],            19],   // 實測 28.05（0.7 時 32.00）
  ['100層 中配 4井cap14 +group', 100,      3,     6,  ['group'],     34],   // 實測 43.23（0.7 時 54.36）
  ['100層 高配 滿級 +dest+group',100,      5,    15,  ['dest','group'], 49],// 實測 58.35（0.7 時 78.33）／cap 16 → 15：#142 之後 cap 的上限就是 15
  ['look+group 3井cap6 45層',     45,      2,     1,  ['group'],     23],   // 實測 31.77（0.7 時 71.44）
  ['dest+group 3井cap6 45層',     45,      2,     1,  ['dest','group'], 20],// 實測 29.41（0.7 時 54.59）
];
...
// ---- #163：飽和 vs 缺陷，要分得開 ----
// `RATE_EXP` 1.0 之下佇列長期壓在 `WAIT_CAP` 上，空車佔比掉到 0.56%。
// 包場**只上空車**（`openDoors` 的規則 2、`soloCalls()` 也只對空車回傳），
// 所以沒有空車 = 這個機制**沒有機會發生**，母體是空的。
//
// ⚠ **那不等於機制壞了，但也不可以算通過。** 三條 guard 在這種時候回 TODO 並
// 指名原因——它每一輪都印出來，不會像綠燈那樣被讀成「沒事」。
//
// ⚠ **只有真的飽和才給 TODO。** 空車還在、卻一次封車都沒有，那是缺陷，照樣紅。
//   分界取 2%：0.7 之下量到的空車佔比是 20–24%，1.0 之下是 0.56%，中間沒有東西。
const EXC_EMPTY_FLOOR = 0.02;
function excSaturated(rows, what){
  const et = rows.reduce((a, r) => a + (r.emptyShaftTicks || r.emptyTicks || 0), 0);
  const tt = rows.reduce((a, r) => a + (r.shaftTicks || 0), 0);
  if (!(tt > 0)) return null;
  const share = et / tt;
  if (share >= EXC_EMPTY_FLOOR) return null;
  return 'TODO: 空車佔比只有 ' + (100 * share).toFixed(2) + '%（門檻 '
    + (100 * EXC_EMPTY_FLOOR) + '%）——包場只上空車，所以這個機制在目前的人流下'
    + '**沒有機會發生**，' + what + '。這不是機制壞了，是 `RATE_EXP` '
    + C.RATE_EXP + ' 把塔淹掉了（#163，owner 裁決）。'
    + '`WAIT_CAP` 或人流一旦回頭，這一條就會自己恢復成真的檢查。';
}
...
check('封車是排他的：封住的車上恰好一個人，而且是他', () => {
  ...
  const sat1 = excSaturated(excDeep, '一台車都沒有被封過');
  if (sat1) return sat1;
...
check('空車調度真的在跑：soloCalls 有回傳、而且包場的人上的是空車', () => {
  ...
  const sat2 = excSaturated(excDeep, '一次包場上車都沒有');
  if (sat2) return sat2;
...
check('劑量反應：crate.w 3/6/12——貨箱數嚴格遞增，設計範圍內封車趟數跟著漲', () => {
  ...
  //   **證偽走這一條。**
  // ⚠ #163：B 只在「有空車可搶」的時候才問得出來。飽和的時候一趟封車都沒有，
  //   那是塔被淹掉不是旋鈕壞了——回 TODO 並指名，不要紅也不要綠。
  //   A（貨箱生成數）不吃這一條：它只看有沒有抽出貨箱，跟空車供給無關，所以照樣守。
  // ⚠ **只讓 B 降級，不要提早 return。** 第一版寫成「飽和就直接回 TODO」，
  //   那會連 A 的失敗一起吞掉——A 紅的時候整條會變成綠底的 TODO。
  //   而且第一版要求「每一格都是 0 趟」，w=12 剛好擠出 1 趟就不成立了；
  //   空車 0.56% 之下那 1 趟是雜訊，不是母體。
  const sat3 = excSaturated(rows, '幾乎每一格都是 0 趟封車');
  const skipB = !!sat3 && rows.some(r => !(r.pool > 0));
  if (!skipB) for (const r of rows)
    if (!(r.pool > 0)) bad.push(`【存活】w=${r.w} 一趟封車都沒有——包場機制在這個人流下是死的`);
  if (bad.length === 0 && skipB) return sat3 + `｜A（旋鈕本體）仍然成立：${line}`;
  return ok(bad.length === 0,
